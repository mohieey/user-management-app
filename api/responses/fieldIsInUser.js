 ...
 *     // -or-
 *     return res.usernameInUse(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function fieldIsInUser(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info("Ran custom response: res.usernameInUse()");
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info(
      "Custom response `res.usernameInUse()` called with an Error:",
      optionalData
    );
